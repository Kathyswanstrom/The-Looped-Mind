...
const CSS = `
  ...
  .post .reply{margin:14px 0 0; padding:14px 16px; background:#0f1d36; border-radius:10px;}
  .post .reply-who{font-family:var(--body); font-size:12px; font-weight:600; letter-spacing:.12em; text-transform:uppercase; color:var(--goldb);}
  .post .reply-body{margin:5px 0 0; font-size:15px; color:rgba(244,239,226,.9); white-space:pre-wrap; font-style:italic;}
          ...
          {posts === null ? (
            ...
            posts.map((p) => (
              <div className="post" key={p.id}>
                ...
                {p.reply ? (
                  <div className="reply">
                    <span className="reply-who">Kathy replies</span>
                    <p className="reply-body">{p.reply}</p>
                  </div>
                ) : null}
              </div>
            ))
